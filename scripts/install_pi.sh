...
if command -v apt-get >/dev/null 2>&1; then
  sudo apt-get update
  sudo apt-get install -y python3 python3-venv python3-pip chromium-browser avahi-daemon || \
    sudo apt-get install -y python3 python3-venv python3-pip chromium avahi-daemon
fi
